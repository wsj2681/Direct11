...
Shader::Shader(ComPtr<ID3D11Device>& device, const wchar_t* vsPath, const wchar_t* psPath, bool isTextured)
{
	isTextured = true;
	// Vertex Shader
	ComPtr<ID3DBlob> vsBlob;
	HR(D3DCompileFromFile(L"textureShader.hlsl", nullptr, nullptr, "VSMain", "vs_5_0", 0, 0, vsBlob.GetAddressOf(), nullptr));
	HR(device->CreateVertexShader(vsBlob->GetBufferPointer(), vsBlob->GetBufferSize(), nullptr, vertexShader.GetAddressOf()));

	// Pixel Shader
	ComPtr<ID3DBlob> psBlob;
	HR(D3DCompileFromFile(L"textureShader.hlsl", nullptr, nullptr, "PSMain", "ps_5_0", 0, 0, psBlob.GetAddressOf(), nullptr));
	...
	if (isTextured)
	{
		...
		HR(wicFactory->CreateDecoderFromFilename(
			L"sample.dds", nullptr, GENERIC_READ, WICDecodeMetadataCacheOnLoad, &decoder));
		...
		// 텍스처 데이터 생성
		std::vector<BYTE> textureData(width * height * 4);
